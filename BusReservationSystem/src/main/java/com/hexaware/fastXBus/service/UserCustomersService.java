...
public class UserCustomersService implements IUserCustomersService {
	...
	@Override
	public Optional<UserCustomers> findByFirstname(String firstName) {
	...
	public List<Bookings> getBookingsForUser(Long userId) {
		Optional<UserCustomers> userOptional =repository.findById(userId);
        if (userOptional.isPresent()) {
            UserCustomers user = userOptional.get();
            return user.getBookings();
        }
        return Collections.emptyList(); // Return empty list if user is not found or has no bookings
